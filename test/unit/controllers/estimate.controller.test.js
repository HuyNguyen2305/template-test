import { beforeEach, describe, expect, jest, test } from '@jest/globals';
import { EstimateController } from '#controllers/estimate.controller.js';

describe('EstimateController', () => {
  let estimateService;
  let controller;
  let reply;

  beforeEach(() => {
    estimateService = {
      getByJobId: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
      remove: jest.fn(),
    };
    controller = new EstimateController({ estimateService });
    reply = {
      status: jest.fn().mockReturnThis(),
      send: jest.fn(),
    };
  });

  test('getByJobId fetches by jobId param and sends the result', async () => {
    estimateService.getByJobId.mockResolvedValue({ id: 10 });
    const request = { params: { jobId: 1 } };

    await controller.getByJobId(request, reply);

    expect(estimateService.getByJobId).toHaveBeenCalledWith(1);
    expect(reply.send).toHaveBeenCalledWith({
      success: true,
      message: 'Estimate fetched',
      data: { id: 10 },
    });
  });

  test('create sends a 201 with the created record', async () => {
    estimateService.create.mockResolvedValue({ id: 10 });
    const request = {
      params: { jobId: 1 },
      body: { estimateNumber: 'EST-1' },
    };

    await controller.create(request, reply);

    expect(estimateService.create).toHaveBeenCalledWith(1, {
      estimateNumber: 'EST-1',
    });
    expect(reply.status).toHaveBeenCalledWith(201);
    expect(reply.send).toHaveBeenCalledWith({
      success: true,
      message: 'Estimate created',
      data: { id: 10 },
    });
  });

  test('update sends the updated record', async () => {
    estimateService.update.mockResolvedValue({ id: 10, poNumber: 'PO-1' });
    const request = {
      params: { jobId: 1 },
      body: { poNumber: 'PO-1' },
    };

    await controller.update(request, reply);

    expect(estimateService.update).toHaveBeenCalledWith(1, {
      poNumber: 'PO-1',
    });
    expect(reply.send).toHaveBeenCalledWith({
      success: true,
      message: 'Estimate updated',
      data: { id: 10, poNumber: 'PO-1' },
    });
  });

  test('remove sends a null-data success response', async () => {
    const request = { params: { jobId: 1 } };

    await controller.remove(request, reply);

    expect(estimateService.remove).toHaveBeenCalledWith(1);
    expect(reply.send).toHaveBeenCalledWith({
      success: true,
      message: 'Estimate deleted',
      data: null,
    });
  });
});
